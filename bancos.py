...
# 60,8:#Regs; 68,17:mtoTot; 93,20:CtaDebito. Detalle: 3,15:CI; 81,17:Monto; 61,20:CtaCliente
  try:
    ...
    if sTPro != ln1[3]:
      ...
      raise ValueError("Error: Rif en columna 44 de 1ra fila errada, deberia ser '" + sRif + "'.")
...
# 60,8:#Regs; 68,17:mtoTot; 93,20:CtaDebito. Detalle: 3,15:CI; 81,17:Monto; 61,20:CtaCliente
  try:
    ...
    if '02' != ln1[0]:
      ...
      raise ValueError("Error: Rif en columna 2 de tercera fila errada, deberia ser '" + sRif + "'")
    elif sEmpresa != ln1[3]:
      raise ValueError("Error: La columna 50 de la tercera fila deberia contener '" + sEmpresa + "', pero contiene: '" + ln1[3] + "'.")
    elif sCodCta != ln1[5]:
...
def vzla(f, sFecha, sCodCta):
  ...
  try:
    ...
    if 'H' != ln1[0]:
      ...
      raise ValueError("Error: La 2da columna de 1ra fila deberia contener '" + sEmpresa + "'; pero contiene: '" + ln1[1] + "'.")
    elif sCodCta != ln1[3]:
...
def mostrarFilas(lTot, nroReg, iNoReg1, iNoReg2, fMtoTot, iMtoTot1, iMtoTot2, sNumLote, sRif, sFechaValor, sCodCta):
...
lTot = calcTotal(lista, nLnCtrl, sLnDet, iCI, iCC, iNac, sTPag, iMto, iNbr)

st = mostrarFilas(lTot, nroReg, iNoReg1, iNoReg2, fMtoTot, iMtoTot1, iMtoTot2, sNumLote, sRif, sFechaValor, sCodCta)
print(st.rstrip(' \t\n\r'))
